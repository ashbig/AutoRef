...
public class RearrayerForPlates {
    ...
    public static final String DEFAULT_LOCATION = Location.UNAVAILABLE;
    ...
    private boolean         m_isOligo = false;
    private boolean         m_isFusion = false;
    private boolean         m_isClose = false;
    private boolean         m_isConvert = false;
    
    private String          m_sample_type = "";
    private String          m_location = DEFAULT_LOCATION;
    ...
    public void                 isOligo(boolean b) {m_isOligo = b;}
    public void                 isFusion(boolean b) {m_isFusion = b;}
    public void                 isClose(boolean b) {m_isClose = b;}
    public void                 isConvert(boolean b) {m_isConvert = b;}
    public void                 setLocation(String l) {m_location = l;}
        ...
        try {
            while((line = in.readLine()) != null) {
                ...
                if (info[3].equals("+") || info[3].equals("1") ) {
                    PlateSample ps = new PlateSample(info[0], info[1], info[2]);
                    if(m_isOligo) {
                        ps.setOligoid5p(Integer.parseInt(info[4]));
                        ps.setOligoid3p(Integer.parseInt(info[5]));
                    }
                    samples.add(ps);
    ...
    FlexWorkflowException, FlexProcessException {
        ...
        Container cont_5 = null;
        Container cont_3f = null;
        Container cont_3c = null;
        
        if(m_isOligo) {
            //create five prime oligo plate
            String plateType = "96 WELL OLIGO PLATE";
            String label = Container.getLabel(projectCode, "OU", threadId, null);
            cont_5 = new Container(plateType, location, label, threadId);
            m_rearrayed_containers.add(cont_5);
            
            //create three prime fusion oligo plate
            if(m_isFusion) {
                label = Container.getLabel(projectCode, "OF", threadId, null);
                cont_3f = new Container(plateType, location, label, threadId);
                m_rearrayed_containers.add(cont_3f);
            }
            
            //create three prime close oligo plate
            if(m_isClose) {
                label = Container.getLabel(projectCode, "OC", threadId, null);
                cont_3c = new Container(plateType, location, label, threadId);
                m_rearrayed_containers.add(cont_3c);
            }
        }
        
        LinkedList org_containers_for_this_container = new LinkedList();
        LinkedList org_oligo_containers_5p = new LinkedList();
        LinkedList org_oligo_containers_3f = new LinkedList();
        LinkedList org_oligo_containers_3c = new LinkedList();
        
        //add positive control samples
        int contId = cont.getId();
        Vector sampleLineageSet = new Vector();
        Vector sampleLineageSet_5p = new Vector();
        Vector sampleLineageSet_3f = new Vector();
        Vector sampleLineageSet_3c = new Vector();
        ...
        for (int count = 0; count < plate_sequences.size(); count ++) {
            ...
            Sample new_sample = new Sample(type, position, cont.getId(),  getConstructid(org_sample), org_sample.getOligoid(), Sample.GOOD);
            cont.addSample(new_sample);
            sampleLineageSet.addElement(new SampleLineage( org_sample.getId(), new_sample.getId()));
            
            if(m_isOligo) {
                Sample org_oligo_5p = new Sample(pl_sample.getOligoid5p());
                Sample org_oligo_3p = new Sample(pl_sample.getOligoid3p());
                Sample new_oligo_5p = new Sample("OLIGO_5P", position, cont_5.getId(), org_oligo_5p.getConstructid(), org_oligo_5p.getOligoid(), Sample.GOOD);
                cont_5.addSample(new_oligo_5p);
                sampleLineageSet_5p.addElement(new SampleLineage(org_oligo_5p.getId(), new_oligo_5p.getId()));
                
                int org_containerid_5p = org_oligo_5p.getContainerid();
                int org_containerid_3p = org_oligo_3p.getContainerid();
                AddToList(org_oligo_containers_5p, org_containerid_5p);
                
                if(m_isFusion) {
                    Sample new_oligo_3p = new Sample("OLIGO_3F",position,cont_3f.getId(),org_oligo_3p.getConstructid(),org_oligo_3p.getOligoid(), Sample.GOOD);
                    cont_3f.addSample(new_oligo_3p);
                    AddToList(org_oligo_containers_3f, org_containerid_3p);
                    sampleLineageSet_3f.addElement(new SampleLineage(org_oligo_3p.getId(), new_oligo_3p.getId()));
                }
                
                if(m_isClose) {
                    Sample new_oligo_3p = new Sample("OLIGO_3C",position,cont_3c.getId(),org_oligo_3p.getConstructid(),org_oligo_3p.getOligoid(), Sample.GOOD);
                    cont_3c.addSample(new_oligo_3p);
                    AddToList(org_oligo_containers_3c, org_containerid_3p);
                    sampleLineageSet_3c.addElement(new SampleLineage(org_oligo_3p.getId(), new_oligo_3p.getId()));
                }
            }
        ...
        if (m_isPutOnQueue) {
            ...
            if(m_isOligo) {
                cont_5.insert(m_conn);
                createSampleLinerageForContainer(cont_5,org_oligo_containers_5p, sampleLineageSet_5p);
                Plateset plateset = null;
                
                if(m_isFusion) {
                    cont_3f.insert(m_conn);
                    createSampleLinerageForContainer(cont_3f,org_oligo_containers_3f, sampleLineageSet_3f);
                    plateset = new Plateset(cont_5.getId(),cont_3f.getId(),-1,cont.getId());
                }
                if(m_isClose) {
                    cont_3c.insert(m_conn);
                    createSampleLinerageForContainer(cont_3c,org_oligo_containers_3c, sampleLineageSet_3c);
                    plateset = new Plateset(cont_5.getId(),-1,cont_3c.getId(),cont.getId());
                }
                plateset.insert(m_conn);
                putPlatesetOnQueue(plateset);
            } else {
    ...
    private void putPlatesetOnQueue(Plateset ps) throws FlexDatabaseException {
        LinkedList queueItems = new LinkedList();
        ...
        for (int i = 0; i < protocols.size(); i++) {
            Protocol pr = (Protocol) protocols.get(i);
            QueueItem queueItem = new QueueItem( ps, pr, m_project, m_workflow);
            queueItems.add(queueItem);
        }
    ...
    protected class PlateSample {
        ...
        public void             setOligoid5p(int id) {oligoid_5p=id;}
        public void             setOligoid3p(int id) {oligoid_3p=id;}
        ...
        public int              getOligoid5p() {return oligoid_5p;}
        public int              getOligoid3p() {return oligoid_3p;}
    }
    ...
    protected int getConstructid(Sample s) throws FlexDatabaseException {
        int constructid;
        
        if(m_isConvert) {
            constructid = s.getConstruct().getPairedConstructid();
        } else {
            constructid = s.getConstruct().getId();
        }
        
        return constructid;
    }
    
    protected void AddToList(LinkedList l, int i) throws FlexCoreException, FlexDatabaseException {
        for(int n=0; n<l.size(); n++) {
            Container c = (Container)l.get(n);
            int e = c.getId();
            if(e == i)
                return;
        }
        
        l.add(new Container(i));
    }
    ...
    public static void main(String [] args) throws Exception {
        ...
        try {
            ...
            rearrayer.setLocation(Location.FREEZER);
