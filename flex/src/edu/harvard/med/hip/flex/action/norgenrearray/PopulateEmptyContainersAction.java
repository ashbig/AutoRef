...
public class PopulateEmptyContainersAction extends ResearcherAction {
    ...
    {
        String sql_check="select label, c.containerid as containerid, q.containerid as qcontainerid,"
+" (select count(sampleid) from sample where containerid=c.containerid) as samplenumber "
+" from queue q, containerheader c ,sample s "
+" where q.containerid(+)=c.containerid and s.containerid(+)=c.containerid   "
  +" and workflowid="+workflowid+" and projectid="+projectid
 +"  and protocolid="+protocolid +" and label =?";
         PreparedStatement stmt = null;ResultSet rs;
        try {
            stmt = conn.prepareStatement(sql_check);
            for( NorgenDestinationPlate dest_plate:   dest_plates)
            {
                stmt.setString(1, dest_plate.getLabel());
                ...
                while (rs.next())
                {
                    ...
                    int qcontainerid = rs.getInt("qcontainerid");
                    if ( qcontainerid != containerid )
                    {
