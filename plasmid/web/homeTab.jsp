<table width="1000" height="49" border="0" align="center">
  <tr> 
    <td height="45"> <table width="100%" height="36" border="0" cellpadding="0" cellspacing="0" bgcolor="#333333">
        <tr> 
          <td><table width="100%" height="33" border="0" cellpadding="4" cellspacing="1" bgcolor="#FFFFFF">
              <tr> 
                <td width="10%" align="center" bgcolor="#6699CC"><strong><font color="#CCCCCC" face="Verdana, Arial, Helvetica, sans-serif">Home</font></strong></td>
                <td width="16%" align="center" bgcolor="#6699CC"><a href="#" STYLE="text-decoration:none"><font face="Verdana, Arial, Helvetica, sans-serif"><strong>About 
                  Us </strong></font></a></td>
                <td width="24%" align="center" bgcolor="#6699CC"><a href="#" STYLE="text-decoration:none"><font face="Verdana, Arial, Helvetica, sans-serif">
                    <strong>Plasmid Submission </strong></font></a></td>
                <td width="33%" align="center" bgcolor="#6699CC"><a href="OrderOverview.jsp" STYLE="text-decoration:none"><font face="Verdana, Arial, Helvetica, sans-serif">
                    <strong>Plasmid Search & Order </strong></font></a></td>
                <td width="17%" align="center" bgcolor="#6699CC"><a href="#" STYLE="text-decoration:none"><font face="Verdana, Arial, Helvetica, sans-serif"><strong>Contact 
                  Us </strong></font></a></td>
              </tr>
            </table></td>
        </tr>
      </table></td>
  </tr>
</table>