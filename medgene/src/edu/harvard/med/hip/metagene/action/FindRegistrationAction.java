...
import edu.harvard.med.hip.metagene.form.FindRegistrationForm;
import edu.harvard.med.hip.metagene.user.*;
import edu.harvard.med.hip.metagene.util.*;
...
public class FindRegistrationAction extends Action {
    ...
    throws ServletException, IOException {
        ...
        String subject = "MedGene registration";
        String msgText = "Information for your MedGene registration"+"\n";
        ...
        try {
            ...
            if ((user_id != null)&& (user_id.length()>0)){
                ...
                else if (manager.reminderUnique(reminder)) {
                    User user = manager.findUser(reminder);
                    if (user!=null){
                        password = user.getPassword();
                        email_address = user.getEmail();
                        //send email
            ...
            else if ((reminder != null)&& (reminder.length()>0) && (manager.reminderUnique(reminder))){
                User user = manager.findUser(reminder);
                if (user!=null){
                    password = user.getPassword();
                    email_address = user.getEmail();
