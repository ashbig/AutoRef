<!DOCTYPE HTML PUBLIC "-//W3C//DTD HTML 4.01 Transitional//EN">
<%@ page errorPage="ProcessError.do"%>
<%@ page language="java" %>
<html>

<head>
<meta http-equiv="Content-Language" content="en-us">
<meta http-equiv="Content-Type" content="text/html; charset=windows-1252">
<title>about BEC Project</title>
</head>

<body>
<jsp:include page="NavigatorBar_Administrator.jsp" />
	<p><P>
<br>
<div align="center">
  <center>
  <table border="0" cellpadding="0" cellspacing="0" width="74%">
    <tr>
        <td width="50%"><font color="#008000" size="5"><b>about BEC Project</b></font></td>
      <td width="50%"></td>
    </tr>
    <tr>
      <td width="100%" colspan="2">
        <hr>
        <p><span style="font-family: Times New Roman; mso-fareast-font-family: Times New Roman; mso-ansi-language: EN-US; mso-fareast-language: EN-US; mso-bidi-language: AR-SA"><span style="mso-spacerun: yes; font-family: Times New Roman; mso-fareast-font-family: Times New Roman; mso-ansi-language: EN-US; mso-fareast-language: EN-US; mso-bidi-language: AR-SA">&nbsp;</span></span></p>
          <p align="left"><a><font size="3" color="#000080">Users can perform
            the following steps:</font></a></p>
        <table width="100%" border="0">
          <tbody>
            <tr>
              <td width="7%" bgColor="#b8c6ed">
                <p align="center">1</p>
              </td>
                <td width="93%" bgColor="#b8c6ed"><font color="#000080" size="3"><a>Transfer
                  plate inforamation from FLEX into BEC</a></font></td>
            </tr>
            <tr>
              <td width="7%" bgColor="#e4e9f8">
                <p align="center">2</p>
              </td>
                <td width="93%" bgColor="#e4e9f8"><font color="#000080"><a></a></font></td>
            </tr>
            <tr>
              <td width="7%" bgColor="#b8c6ed">
                <p align="center">3</p>
              </td>
                <td width="93%" bgColor="#b8c6ed">&nbsp;</td>
            </tr>
            <tr>
              <td width="7%" bgColor="#e4e9f8">
                <p align="center">4</p>
              </td>
                <td width="93%" bgColor="#e4e9f8"><font color="#000080"><a></a></font></td>
            </tr>
            <tr>
              <td width="7%" bgColor="#b8c6ed">
                <p align="center">5</p>
              </td>
                <td width="93%" bgColor="#b8c6ed">&nbsp;</td>
            </tr>
            <tr>
              <td width="7%" bgColor="#e4e9f8">
                <p align="center">6</p>
              </td>
                <td width="93%" bgColor="#e4e9f8"><font color="#000080"><a></a></font></td>
            </tr>
          </tbody>
        </table>
          <p><font color="#000000"><span style="mso-spacerun: yes; font-family: Times New Roman; mso-fareast-font-family: Times New Roman; mso-ansi-language: EN-US; mso-fareast-language: EN-US; mso-bidi-language: AR-SA">If
            you have some question, please contact <a href="mailto:elena_taycher@hms.harvard.edu">BEC
            development Group</a>.</span></font></td>
    </tr>
  </table><br><br>
  <a href="login.jsp"><img border="0" src="jpg/home.gif"></a>
  </center>
</div>

</body>

</html>

